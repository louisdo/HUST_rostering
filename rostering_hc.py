...
def check(data:np.array, shift_count: np.array, params: dict):
    ...
    for d in range(params["D"]):
        for s in range(1, 5):
            #ce = encouragement_score(shift_count, d, s, params)
...
def choose(fs: dict, avail_shifts: list):
    ...
    unnormed_probs = np.array([fs[s] for s in avail_shifts])
    return avail_shifts[np.argmax(unnormed_probs).item()]
...
if __name__ == "__main__":
    ...
    with open("./output_hc.json", "w") as f:
        json.dump({
